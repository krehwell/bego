...
func getGrid() [][]int {
    ...
    for i := 0; i <= len(rawGridSplit) - gridRowLen; i += gridRowLen {
        ...
        for j := 0; j < gridRowLen; j++ {
            ...
            if rawGridNum[0] == '0' {
                rawGridNum = string(rawGridNum[1])
            }

            num, err := strconv.Atoi(rawGridNum)
            if err != nil {
...
func getBiggestForDiagonalDownRight(series [][]int) int {
    max := 0

    for i := len(series) - 1; i >= 0 + adjacent; i-- {
        tempMax := 0
        for j := len(series[i]) - 1; j >= 0 + adjacent; j-- {
            num1 := series[i][j]
            num2 := series[i - 1][j - 1]
            num3 := series[i - 2][j - 2]
            num4 := series[i - 3][j - 3]
